...
// ★ (2026-06-15, dspark): 그리드 셀 검색 picker (G14) — childObjId 를 이름으로 검색→ID 자동채움.
//   col.picker = { title, placeholder, columns, fetcher, rowKey, valueKey, displayValueKey }
...
function onPickerSelect(sel) {
  ...
  if (col && row) {
    ...
    if (row.rowStatus !== 'I') row.rowStatus = 'U';   // 기존행 편집 마크 (touch 대체)
  }
...
const relColumns = [
  ...
  // ★ (2026-06-15, dspark): 숫자 직접입력 → 검색 picker(G14). 이름으로 찾아 OBJECT_ID 자동채움 + 이름 표시.
  { key: 'childObjId', label: '하위 오브젝트', kind: 'picker', width: 240, displayKey: 'childObjectNm',
    picker: {
      title: '오브젝트 검색', placeholder: '오브젝트ID 또는 화면명',
      columns: [{ key: 'objectNm', label: 'OBJECT_NM', width: 160 }, { key: 'objectDisplayNm', label: '화면명' }],
      fetcher: (q) => adminApi.meta.objects.list({ q, size: 50, page: 1 }).then((r) => r.content || []),
      rowKey: 'objectId', valueKey: 'objectId', displayValueKey: 'objectNm',
    } },
];
...
<template>
  ...
  >
    ...
    <template #drawer>
      ...
      >
        ...
        <section v-else-if="drawerTab === 'relations'" class="section">
          <MetaChildGrid
            ...
            :new-row="newRel"
            add-label="+ 하위 추가"
            hint="이 오브젝트가 합성하는 하위 오브젝트(팝업/탭). 하위 오브젝트 칸의 🔍 로 이름 검색→선택하면 OBJECT_ID 가 자동 채워집니다."
      ...
      <!-- ★ (2026-06-15) 그리드 셀 검색 picker (하위 오브젝트) -->
      <SearchPickerModal
        v-if="pickerCol"
        :open="pickerOpen"
