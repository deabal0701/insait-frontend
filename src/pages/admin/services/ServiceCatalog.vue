...
// ★ (2026-06-15, dspark): 인라인 서버검색(searchcombo) 설정 — 팝업 없이 칸 타이핑→리스트→선택(MetaChildGrid self-contained).
const SC_SQL = {   // func_nm = SQL 쿼리명
  fetcher: (q) => adminApi.meta.queries.list({ q, size: 50, page: 1 }).then((r) => r.content || []),
  value: (raw) => raw.queryName,
  label: (raw) => `${raw.queryName}${raw.displayName ? ' — ' + raw.displayName : ''}`,  // 드롭다운(풍부)
  short: (raw) => raw.queryName,                                                        // 접힌 표시값(짧게)
  apply: (raw, row) => { row.funcNm = raw.queryName; },
};
const SC_MSG_VALUETYPE = {   // value_type = MT_ 메시지ID
  fetcher: (q) => adminApi.meta.messages.list({ q, size: 50, page: 1 }).then((r) => r.content || []),
  value: (raw) => raw.msgDefId,
  label: (raw) => `${raw.msgDefId}${raw.msgDefNm ? ' — ' + raw.msgDefNm : ''}`,  // 드롭다운(풍부)
  short: (raw) => raw.msgDefId,                                                   // 접힌 표시값(짧게)
  apply: (raw, row) => { row.valueType = raw.msgDefId; },
};
const SC_MSG_ATTRNM = {   // 속성명 = ME_ (MT_ 선택→ME_ 자동변환 + value_type 동시 채움)
  fetcher: (q) => adminApi.meta.messages.list({ q, size: 50, page: 1 }).then((r) => r.content || []),
  value: (raw) => 'ME_' + (raw.msgDefId || '').slice(3),
  label: (raw) => `${raw.msgDefId} → ME_${(raw.msgDefId || '').slice(3)}`,  // 드롭다운(풍부)
  short: (raw) => 'ME_' + (raw.msgDefId || '').slice(3),                     // 접힌 표시값(짧게)
  apply: (raw, row) => { const id = raw.msgDefId || ''; row.svAttrNm = 'ME_' + id.slice(3); if (!row.valueType) row.valueType = id; },
};
...
const funcMapColumns = [
  ...
  // ★ (2026-06-15, dspark): SQL 인라인 검색(searchcombo) — 칸 타이핑→쿼리 리스트→선택.
  { key: 'funcNm',      label: 'func_nm (쿼리/엔터티명)', kind: 'searchcombo', placeholder: 'SQL 쿼리 검색', sc: SC_SQL },
...
const attrColumns = [
  // ★ (2026-06-15, dspark): 속성명(ME_) = 메시지 인라인 검색(MT_) → ME_ 자동변환 + value_type(MT_) 동시 채움. 한 번 선택으로 ME_/MT_ 페어 완성.
  { key: 'svAttrNm',     label: '속성명 (ME_*)', kind: 'searchcombo', placeholder: '메시지 검색 → ME_', sc: SC_MSG_ATTRNM },
  { key: 'svAttrType',   label: '타입', kind: 'select', width: 110,
    options: [{ value: 'IN_MSG', label: 'IN_MSG' }, { value: 'OUT_MSG', label: 'OUT_MSG' }] },
  // value_type(MT_) = 메시지 인라인 검색 → msgDefId 채움.
  { key: 'valueType',    label: 'value_type (MT_*)', kind: 'searchcombo', placeholder: '메시지 검색', sc: SC_MSG_VALUETYPE },
      ...
      >
        ...
        <section v-else-if="drawerTab === 'attrs'" class="svc-section">
          <MetaChildGrid
            ...
            :new-row="newAttr"
            add-label="+ 속성 추가"
            hint="속성명·value_type 칸에 메시지ID/한글명을 타이핑하면 검색 리스트에서 선택할 수 있고, 속성명은 ME_, value_type 은 MT_ 로 자동 채워집니다. (IN_MSG/OUT_MSG 슬롯)"
          />
        ...
        <section v-else-if="drawerTab === 'funcMaps'" class="svc-section">
          <MetaChildGrid
            ...
            :new-row="newFuncMap"
            add-label="+ 매핑 추가"
            hint="sql=쿼리명(조회/프로시저) / entity=엔터티명(저장). func_nm 칸에 SQL 이름을 타이핑하면 검색 리스트에서 선택할 수 있습니다."
          />
          <ul v-else-if="detail" class="resource-list">
